...
export default function MobileNav({ nav }: { nav: NavItem[] }) {
    ...
    // アニメーション制御：openがtrueになったら少し遅延して表示
    useEffect(() => {
        if (open) {
    ...
    return (
        <>
            ...
            >
                <span
                    className={`absolute h-[2px] w-5 bg-current transition-all duration-300 ${open ? "rotate-45 translate-y-0" : "-translate-y-2"
                        }`}
                />
                <span
                    className={`absolute h-[2px] w-5 bg-current transition-all duration-300 ${open ? "opacity-0 scale-0" : "opacity-100 scale-100"
                        }`}
                />
                <span
                    className={`absolute h-[2px] w-5 bg-current transition-all duration-300 ${open ? "-rotate-45 translate-y-0" : "translate-y-2"
            ...
            {open && (
                <div className="fixed inset-0 z-[60]">
                    ...
                    {/* パネル */}
                    <div
                        className={`absolute right-0 top-0 h-full w-[86%] max-w-sm border-l border-[#ddc9a3] bg-[#fff8e7] shadow-xl transition-transform duration-300 ease-out ${isVisible ? "translate-x-0" : "translate-x-full"
                            }`}
                    >
                                ...
                                {nav.map((item, index) =>
                                    item.external ? (
                                        <a
                                            ...
                                            onClick={handleClose}
                                            className={`mobile-nav-item flex items-center justify-between rounded-xl border border-[#ddc9a3] bg-white px-4 py-3.5 text-sm font-medium text-[#3d2f24] transition-all hover:bg-[#fff8e7] hover:border-[#b87333] active:scale-[0.98]`}
                                            style={{
                                        ...
                                        <Link
                                            ...
                                            href={item.href}
                                            onClick={handleClose}
                                            className={`mobile-nav-item block rounded-xl border border-[#ddc9a3] bg-white px-4 py-3.5 text-sm font-medium text-[#3d2f24] transition-all hover:bg-[#fff8e7] hover:border-[#b87333] active:scale-[0.98]`}
                                            style={{
                                                animationDelay: isVisible ? `${index * 50}ms` : "0ms",
                                                opacity: isVisible ? 1 : 0,
                                            }}
